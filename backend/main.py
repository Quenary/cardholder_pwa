...
def refresh_token(refresh_token: str):
    username = auth.validate_refresh_token(refresh_token)
...
def get_card(card_id: int, db: Session = Depends(auth.get_db), user=Depends(auth.get_current_user)):
...
def create_card(card: schemas.CardCreate, db: Session = Depends(auth.get_db), user=Depends(auth.get_current_user)):
...
def update_card(card_id: int, updated: schemas.CardCreate, db: Session = Depends(auth.get_db), user=Depends(auth.get_current_user)):
    card = db.query(models.Card).filter_by(id=card_id, owner_id=user.uid).first()
    if not card:
        raise HTTPException(status_code=404, detail="Card not found")
...
def delete_card(card_id: int, db: Session = Depends(auth.get_db), user=Depends(auth.get_current_user)):
    card = db.query(models.Card).filter_by(id=card_id, owner_id=user.uid).first()
    if not card:
        raise HTTPException(status_code=404, detail="Card not found")
    db.delete(card)
